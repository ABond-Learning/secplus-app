// Build an SB-fix-1b review packet from SB1.6 verdicts.
//
// Filters D2 partial-adjacent items in match + cram types only (per supervisor
// TASK 3 sign-off — mc + scen already shipped in SB-fix-1a). Sorts
// deterministically by (section, video, type, index) so packets are stable
// across re-runs.
//
// For each item produces a markdown review row with: location header, parent
// video info, current citation, item content (prompt/answer for match,
// term/def for cram), LLM verdict, full justification, parser-suggested
// destinations, and Aiden-facing decision checkboxes.
//
// Schema: per-item citation override on MatchItem + CramTerm was added in
// SB-fix-1b-prep (c1664c0). Both-or-neither rule applies. Apply script
// writes messerVideo + subObjective + audit_d_review block.
//
// Usage:
//   node scripts/sb-fix-1b-build-packet.mjs --packet 1 --start 0 --size 25
//   node scripts/sb-fix-1b-build-packet.mjs --packet 2 --start 25 --size 25
//   ...
//   node scripts/sb-fix-1b-build-packet.mjs --packet 6 --start 125 --size 9
//
// Output paths:
//   .audit-working/sb-fix-1b/packet-N.md  (Aiden review, clipboard-relay format)
//   .audit-working/sb-fix-1b/packet-N.json (structured shadow for the apply script)
...
const OUT_DIR = resolve(repo, ".audit-working/sb-fix-1b");
...
// Filter: D2 partial-adjacent, match + cram only, sorted deterministically
const scope = verdicts
  .filter(r =>
    r.verdict?.category === "partial-adjacent" &&
    r.location?.section?.startsWith("2.") &&
    (r.location.type === "match" || r.location.type === "cram")
  )
  .sort((a, b) => {
...
// ─── Destination parser (allowlist-based, parser v2 verbatim from SB-fix-1a) ─
//
...
function renderItemContent(item, type) {
  // match/cram have smaller, fixed shapes — render directly.
  const out = [];
  out.push("```");
  if (type === "match") {
...
function renderRow(row, n) {
  ...
  // For match/cram, current citation usually inherits from parent (no per-item
  // override) — render "(inherits parent)" when fields absent.
  const currentMesser = item.messerVideo || `(inherits parent: ${l.section} - ${parentTitle})`;
  const currentSubObj = item.subObjective || `(inherits parent: ${l.section})`;
  // Pass the PARENT video citation to parser so the partial-adjacent
  // destination-naming filter excludes the parent video (which the LLM
  // typically names as the failing source).
  const parentCitation = `${l.section} - ${parentTitle}`;
  const filterFromCitation = item.messerVideo || parentCitation;
  const dests = parseDestinations(v.justification_explanation || "", filterFromCitation);
  ...
  out.push("");
  out.push(`**Parent video:** ${l.section} - ${parentTitle}`);
  out.push(`**Currently cited as:** \`${currentMesser}\` → \`${currentSubObj}\``);
  out.push("");
  out.push("**Item content:**");
  out.push("");
  for (const line of renderItemContent(item, l.type)) out.push(line);
  out.push("");
...
const lines = [];
lines.push(`# SB-fix-1b packet ${args.packet} — Domain 2 partial-adjacent re-citation review (match + cram)`);
lines.push("");
lines.push(`Generated: ${new Date().toISOString()}`);
lines.push(`Source: \`.audit-working/audit-d-sub-batch-1/full-corpus-verdicts-sb16.json\``);
lines.push(`Filter: \`category=partial-adjacent\` AND \`location.section startsWith "2."\` AND \`type in {match, cram}\``);
...
lines.push("");
lines.push("Match items show `Prompt` (left-column phrase) → `Answer` (right-column phrase). Cram items show `Term` → `Def`. Match/cram have less content per item than mc/scen — review tempo should be faster.");
lines.push("");
lines.push("Decision options:");
lines.push("- **accept primary** — parser's first suggestion is the right destination. Apply script will write `item.messerVideo` (new per-item override field, added in SB-fix-1b-prep) and `item.subObjective` to the named target.");
...
lines.push("- **reject** — LLM was wrong; item really does belong where currently cited (i.e. inherits parent correctly). Apply script will write `audit_d_review.kept_as_is = true` so the item won't be re-flagged in future audit passes.");
...
lines.push(`- Parser-yielded primary destination available on: ${slice.filter(r => {
  ...
  const parentCit = `${r.location.section} - ${located.parentVideoTitle}`;
  const filter = located.item?.messerVideo || parentCit;
  return parseDestinations(r.verdict.justification_explanation || "", filter).length > 0;
}).length} / ${slice.length}`);
lines.push(`- match items: ${slice.filter(r => r.location.type === "match").length}`);
lines.push(`- cram items: ${slice.filter(r => r.location.type === "cram").length}`);
const sectionCounts = {};
...
const shadow = {
  ...
  items: slice.map((row, i) => {
    ...
    const parentCit = located ? `${row.location.section} - ${located.parentVideoTitle}` : null;
    const filter = located?.item?.messerVideo || parentCit;
    const dests = parseDestinations(row.verdict.justification_explanation || "", filter);
    return {
      packet_index: args.start + i + 1,
      location: row.location,
      parent_video_title: located?.parentVideoTitle || null,
      parent_citation: parentCit,
...
console.log("");
console.log(`match items: ${slice.filter(r => r.location.type === "match").length}, cram items: ${slice.filter(r => r.location.type === "cram").length}`);
