...
> - **`f7de013` — commit 3: ConfidenceRater UI.** 4-button pre-check confidence (`q`/`w`/`e`/`r` → 0..3 = no idea / guessed / fairly sure / certain), optional/skippable, captured before Check, written onto the `weakness-` record (matching + exam stay null by design). Honoured the signed-off **Q-B-3 4-button** scale (`9c5df20`) over the ship prompt's stale 3-button text — PLAN.md feature #1 predated Q-B-3 and was corrected at source this commit. Also closed an **inherited CI gap**: `package.json` test glob now includes `src/study/__tests__/`, so commit-2's 15 weakness tests (un-enforced between `0b831ba` and `f7de013`) now run. Report-#0026.
> - **`5c0ed41` — Option B Piece 2: `fetchGist` truncation safety (ungated integrity fix).** Past GitHub's ~1 MB gist inline-content truncation the old code parsed partial JSON, threw, and returned an EMPTY payload → `doSync` kept local + re-pushed → cross-device reconciliation silently stopped for ALL data with no error. Fix: on `truncated`, recover via `raw_url`; on unrecoverable/present-but-unparseable, throw a loud `lastError` (degraded) instead of silent-empty; `deriveHealth` red-regex extended so it shows red immediately. Absent file still legitimately empty. Report-#0027.
> - **`4a64a51` — Option B Piece 1: `weakness-` reclassified `LOCAL_ONLY` (shipped ALONE, gated).** Stops live-syncing unbounded per-attempt weakness data that was growing the Gist payload toward the truncation threshold. Already-pushed records pruned automatically by `mergeEntries`' isTracked-drop on next push (local copies kept — export still sees them). **Four** old-behaviour tests inverted (ship prompt said two; full-suite run found four — flagged per verify-cited-baselines). Report-#0028.
>
> **Three-part gate CLEARED 2026-06-01.** All three devices hard-reloaded onto the new LOCAL_ONLY bundle and confirmed green; Gist `a580a7dbcda2957a28de0f89881b5586` verified **weakness-clean** (0 `weakness-` keys; payload holds only `secplus-v4` + `secplus-v4-customise-last`). **Commit 5 is UNBLOCKED.**
>
> **Behavioural consequence:** weakness records are now **local-only per device** until commit 5 (import/export, Q-F-1) ships — that IS the designed Q-F cross-device path for weakness data now that live-sync is off. Until then, a device only has the weakness records written on it.
>
> **Task 1h position:** commits 1-3 shipped; Option B sync-hardening shipped + gate cleared; **commit 5 (import/export) next + unblocked**; commits 4 (pause-on-blur Q-C-3), 6 (SCHEMA/docs), 7 (Report) pending. `npm test` 79/79, validator 0 errors, build clean at HEAD.
>
> **Next-session carry-forwards:** (a) `deriveHealth` + `formatTime` → extract to a plain `sync-health.js` so `node --test` can unit-test the health logic (Piece 2's red-regex is currently covered only transitively) — queued for commit 6's slot, not built. (b) After ~3-5 real study sessions, query the fraction of `weakness-` records carrying a `confidence` value to decide whether the optional rating needs a soft nudge — study-surfaced decision, nothing built speculatively. Rule #12 + Rule #9 both applied as written today; no new workflow rule emerged (CLAUDE.md unchanged).
>
> *(Supersedes the 2026-05-29 banner below.)*

# Where Things Stand — 2026-05-29 (1g arc CLOSED — data f39c358 / docs 75d866d / display this commit)

> **2026-05-29 — Task 1g.3 Sybex fold-in CLOSED.** Three-commit arc: data half at `f39c358` (499/500 items + SM-2 keys + 5 framing_notes + regression test); doc/closeout half at `75d866d` (5-file SM-2 key-order doc fix + Report-#0024 L1-L6 limitations + erratum footers on Reports #0022 + #0023); display half at this commit (`framing_note` render hook in `src/secplus-quiz.jsx` + companion `styles.framingNote` + 3-gate browser smoke PASS — pre-check leak / post-check display / non-Sybex no-show). 1g.8 plan adjudication tightened Q-C from "Sybex framing note" (source citation) to "Exam framing:" (exam-relevance lead). Report-#0025 captures the final ship + 1g arc close. Six known limitations carried as documented baseline (L1-L6 of Report-#0024; L6 closed by this commit). Six Rule #12 catches across the 1g arc, all on doc/spec-vs-HEAD drift — three pre-2026-05-29 (1g.1 stale 5-warns baseline / 1g.4 SM-2 prefix order pre-dry-run / 1g.6 mcKey app-side wiring not deferred) and three on 2026-05-29 (1g.7 SM-2 key-order pre-stage doc fix / Report-#0024 base-SHA a7ea18a HEAD-vs-ship clarification / 1g.8 framing_note display gap). Attribution: the strengthened "leading sentence" practice (`a39bd35`) caught the SM-2 key-order pre-stage check; Report-#0024 base-SHA was a supervisor verification flag; the framing_note display gap was Aiden's browser smoke at `2.2.sybex` ch04-q1 (not a Rule #12 pre-flight catch — it was the visual-smoke gate firing on a feature surface that automated tests couldn't certify). **Next:** Task 1 closeout (separate scoping conversation); Task 2 mode consolidation continues; weakness-tracker (Task 1h) commits 3-8 pending. *(Supersedes the 2026-05-28 banner below.)*

> **2026-05-28 session close — Task 1g.3 Sybex fold-in: corpus tagged (500, $3.04), full blind second-reader check passed (84.2% strict / 91.0% domain-collapse, disagreements on defensible boundary seams), 1g.4-1g.6 SHIPPED at f39c358 — 499 items merged across 28 synthetic X.Y.sybex videos, Option A SM-2 keys (sybex-mc-chNN-qN) wired app-side, 5 framing_notes attached, regression test confirms zero existing-key drift. OUTSTANDING: (a) visual UI smoke in browser — synthetic Sybex videos render, study modes include them, framing_note surfaces; (b) 1g.7 Report-#NNNN with known limitations (ambiguity_flag under-fires, confidence non-discriminating, 4.4-over-4.9 judge lean, 1/500 skipped sybex-ch02-q19, boundary seams 3.2↔4.5/2.x↔4.x/2.5↔4.1). 1g.3 closes when 1g.7 lands. Three Rule #12 catches this arc, all on doc/spec-vs-HEAD drift.** *(Closed by the 2026-05-29 banner above — 1g.7 + 1g.8 shipped.)*
