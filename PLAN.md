...
**1f — Audit D: citation grounding** | **SB0 SHIPPED 2026-05-13 `111be1f`; SB1 pre-flight SHIPPED 2026-05-14 `aa32fad`; SB1.5 post-process SHIPPED 2026-05-18 `a26d42c`; SB1 full-corpus ATTEMPTED-AND-HALTED 2026-05-18 (resume infra `1a5798c` + `a4a30c3`)** | Hybrid keyword pre-screen + LLM-as-judge + Aiden arbitration. SB0: 5 scripts + 30-item calibration, $0.32; smoke PASSED; strict 76.7% / collapsed 86.7%. SB1 pre-flight: tuned prompt + cache + verbatim-retry; Rec 2 partial-depth examples landed (3/3 targeted shifts); Rec 4 verbatim-retry landed (paraphrase 27%→8.3%); Rec 1 partial-adjacent attempted in iter1 (reorder + decision-tree + consistency check) but broke smoke + 10/23 regressions → rolled back; Rec 3 confidence accepted as limitation. Pass-criteria 5/7 PASS, 2 FAIL (both Rec 1 not biting). Spend $0.97. **SB1.5: `scripts/audit-d-postprocess-verdicts.mjs` (~140 lines incl. comments) flips category to partial-adjacent whenever fix_direction=move-to-correct-video. Spot-check on iter0 micro-recal: 3 flips (rows 10, 19, 24 — all subset-1-disagree). Subset 1 strict 3/6 → 6/6 (PASS). Subset 1 collapsed 6/6 (held). Subset 2 unchanged 3/5 strict (residual: HMAC + CCPA — neither had fix_direction=move-to-correct-video). Internally inconsistent verdicts 3 → 0. On regression set: 2 additional flips (§2.3.9 hypervisor + §2.3.2 integer overflow — both had iter0 prose explicitly identifying partial-adjacent reasoning) — these read as SB0-supervisor-too-conservative, not script error. Smoke-test invariant (§2.3.3 mutex/atomic) HELD. $0 (no LLM call). API credit ~$53.71. **Next: SB1 full corpus (scope + model decision, then run).** Closure docs `docs/audit-d-calibration-summary.md` (SB0), `Reports/Report-#0002.md` (SB0), `Reports/Report-#0003.md` (SB1 pre-flight), `Reports/Report-#0004.md` (SB1.5 — covers full 2026-05-18 session incl. matching-bug fix). Subsumes Audit B; defers Audit C indefinitely; supersedes Task 1c-structural.
...
**Sub-batch 1 full corpus — ATTEMPTED 2026-05-18, HALTED at call #688 / $7.42 due to laptop travel.** Pre-flight signed off with scope=match+cram+MC+scen (N=2128 sample-sampled), model=sonnet-4-5 (no SB1.5 cross-run), HARD_CAP=3000, mid-projection $30 / stretch $45 under $60 ceiling. Run launched, completed 688 calls / $7.42 before halt. Original script wrote verdicts only at end → $7.42 sunk (no on-disk verdicts). **Resume patch landed 2026-05-18 `a4a30c3`** (`scripts/audit-d-llm-judge.mjs` now reads existing `--output`, skips done locations by `section|video|type|index`, flushes every 50 verdicts; smoke-tested with fake 2128-verdict file → 0 API calls). Sample builder `scripts/audit-d-build-sub-batch-1-sample.mjs` shipped in `1a5798c`. **Restart tomorrow in fresh CC session**: re-run `node scripts/audit-d-llm-judge.mjs --input .audit-working/audit-d-sub-batch-1/full-corpus-sample.json --output .audit-working/audit-d-sub-batch-1/full-corpus-verdicts.json`. Then apply audit-d-postprocess-verdicts → surface flip-rate stats (raw vs postprocessed, per-domain) → SURFACE-AND-HOLD for Aiden review packet.
