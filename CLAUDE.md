...
12. **Critical review of supervisor ship prompts.** Before executing any supervisor ship prompt, review it critically: validate its figures, claims, scope, and any destructive or irreversible steps against live state. If you find a concern — stale figure, factual error, scope mismatch, missing safety check, risky op — push back via paste-relay before executing, not after. If the prompt is sound, proceed. This rule does not require producing a critique on every prompt: silence-and-proceed is the correct response to a sound prompt, substantive pushback the correct response to a flawed one.
